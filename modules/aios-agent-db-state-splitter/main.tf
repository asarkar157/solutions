  ...
  stage_bindings = [
    {
      ...
      note = <<-EOT
        ...
        **Subagent discipline:** delegate this stage to ONE child subagent named exactly `ingest-monolith-runner`, `task_type="terminal_calling"`, spawn goal ≤ 1000 chars (script paths only — never inline `jq`/bash). No `-v2`/`-scripts`/etc. suffixes; re-plan instead of re-trying. See db-state-split-orchestration-sop **Subagent-spawn discipline**.
        ...
        1) read_notes (then `cat notes.json` if present); **clone IaC repo using env-mounted git credentials** — follow db-state-split-orchestration-sop § *Git connectivity* exactly: parse host from `iac_repository_url`, prefer `GIT_TOKEN_<HOST>` then `GIT_TOKEN` (legacy `GITHUB_TOKEN` still honored), fall back to `GIT_SSH_PRIVATE_KEY` + `GIT_SSH_KNOWN_HOSTS`. Use `git -c credential.helper=` + URL rewrite so the token never lands in `~/.git-credentials`. **Never** echo `$GIT_TOKEN` / `$GIT_SSH_PRIVATE_KEY` into `note` / `notify` / chat / logs. If the env is empty and clone returns 401/403/`could not read Username`, `notify` once with `{stage:'ingest-monolith', error:'git_credentials_missing', host:"$GIT_URL_HOST"}` and **return** — do NOT `ask_clarifying_question` for a token. If clone/update fails for filesystem reasons (read-only, permission denied), use a fresh directory under `/tmp` and `note` the real `repo_clone_path`. If workflow inputs include `grouping_policy_json`, `note` it under key `grouping_policy_json`.
