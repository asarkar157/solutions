...
Guild prepends **`[Runbook Context]`** / **`### Runbook:`** sections — that text is **authoritative**. Never call `search_skill` to replace or "re-find" those SOPs. If a `[Skills]` line names a document already present as a `### Runbook:` heading, skip `load_skill`. Call `load_skill` only for extra skill names with no matching runbook section. Use `search_tools` only for a new MCP prefix.
...
- **Git credentials live in Ubuntu env, same as AWS creds.** Operators wire `sg_secret` → Ubuntu `secret_ref_ids` so `GIT_TOKEN` / `GIT_HOST` / `GIT_USERNAME` (or `GIT_SSH_PRIVATE_KEY` + `GIT_SSH_KNOWN_HOSTS`) are mounted on the container. Use the detection + clone block from **db-state-split-orchestration-sop** § *Git connectivity*. **Never** echo tokens into `note` / `notify` / chat / logs; never `ask_clarifying_question` for a token — if env is missing, single `notify({error:'git_credentials_missing', host:...})` and return.
...
- **Execution Optimization Protocol — batch Ubuntu CLI calls.** When a stage step needs more than one related command (`mktemp` + `chmod` + `touch`, `tofu init` + `plan` + `show`, jq pipeline + write-back, …), you **MUST** issue them as **one** `ubuntu-cli_execute_series` call. **Never** fire concurrent or rapid-succession `ubuntu-cli_execute_command` calls in the same turn (audit pattern: 8 simultaneous `execute_command` calls at one timestamp). `ubuntu-cli_execute_command` is for a **single** cohesive command (e.g. `cat /tmp/.../notes.json` at stage entry). For **independent** per-group / per-shard fan-out, use `ubuntu-cli_execute_parallel` (or `flow_type:"parallel"` subagent children), never N parallel `execute_command` calls. Prefer one script via `ubuntu-cli_create_files` + one `execute_series` over many ad-hoc commands. See **db-state-split-orchestration-sop** § *Execution Optimization Protocol*.
...
- **AppStack membership is a closed set.** For every `group_id`, added resources must equal `resource_addresses[]` — no missing, no extras, no cross-group bleed. After each per-group `add_resource_to_appstack` loop, call `get_appstack_resources(appstack_id)` and write `stackgen_appstack_membership:<group_id>` JSON with `ok=true|false`; reconcile until `ok=true` before `connect_resources` or Plan. Roll up into `stackgen_appstack_membership_report` (required evidence). Never re-bucket by Terraform type at MCP time — `logical_group_manifest` decides. See **stackgen-appstack-mcp-playbook-sop** step 3.5.
- **Env profile + StackGen Plan are OPTIONAL, never blocking.** Project envs cannot be created via this MCP. With `stackgen_target_environment` unset, skip both and write `{skipped:"no_target_env_input"}` / `{skipped:"no_env_profile"}` notes. On `environment '<env>' not found in project settings`, soft-fail with `stackgen_mcp_errors{reason:"env_not_in_project_settings"}` + same skipped notes. Single `notify` per workflow run only when `stackgen_environments_required="true"`. Skipped env / Plan never makes membership `ok=false`.
- **Secrets and telemetry.** State can contain secrets. Do **not** paste raw `instances[].attributes` into `note`, chat, or `notify` — store paths, counts, hashes, redacted summaries only. Treat `/tmp` as sensitive on shared runners (`chmod 700`); avoid world-readable copies of state.
