output "policy_create_flags" {
  ...
  value = {
    ...
    spec_traceability            = try(var.create_policies.spec_traceability, true)
    sre_investigation_write_gate = try(var.create_policies.sre_investigation_write_gate, true)
    pagerduty_escalation_gate    = try(var.create_policies.pagerduty_escalation_gate, true)
  }
}
...
output "policy_ids" {
  ...
  value = {
    ...
    spec_traceability            = length(sg_policy.spec_traceability) > 0 ? sg_policy.spec_traceability[0].id : ""
    sre_investigation_write_gate = length(sg_policy.sre_investigation_write_gate) > 0 ? sg_policy.sre_investigation_write_gate[0].id : ""
    pagerduty_escalation_gate    = length(sg_policy.pagerduty_escalation_gate) > 0 ? sg_policy.pagerduty_escalation_gate[0].id : ""
  }
}
...
output "policy_names" {
  ...
  value = {
    ...
    spec_traceability            = length(sg_policy.spec_traceability) > 0 ? sg_policy.spec_traceability[0].name : ""
    sre_investigation_write_gate = length(sg_policy.sre_investigation_write_gate) > 0 ? sg_policy.sre_investigation_write_gate[0].name : ""
    pagerduty_escalation_gate    = length(sg_policy.pagerduty_escalation_gate) > 0 ? sg_policy.pagerduty_escalation_gate[0].name : ""
  }
}
