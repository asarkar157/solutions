#!/usr/bin/env bash
# Minimal cdk-bot clone pack — embed via ONE execute_series command:
#   /bin/bash -s clone WORK_ROOT URL BRANCH ISSUE PR_REF PR_URL <<'CDKBOT_CLONE_PACK'
#   ...this file...
#   CDKBOT_CLONE_PACK
...
# assert_clone_arg_literal rejects spawn-context mistakes where agents pass
# single-quoted '$REPO_CLONE_URL' argv to clone-pack.sh (trace 0bf8c7ef).
assert_clone_arg_literal() {
  local label="${1:?label}"
  local value="${2:?value}"
  local work_root="${3:?work_root}"
  case "$value" in
    '$REPO_CLONE_URL'|'$DEFAULT_BRANCH'|'$ISSUE_OR_PR'|REPO_CLONE_URL|DEFAULT_BRANCH|ISSUE_OR_PR)
      mkdir -p "$work_root"
      [ -f "$work_root/notes.json" ] || echo '{}' >"$work_root/notes.json"
      mirror_note "$work_root" "clone_blocker" "unexpanded_shell_var"
      echo "clone_blocker=unexpanded_shell_var"
      echo "bad_arg=${label}"
      echo "hint=copy_spawn_context_clone_verbatim_literal_url_branch_issue_argv"
      exit 1
      ;;
  esac
}
...
cmd_clone() {
  ...
  assert_clone_arg_literal "repository_clone_url" "$repo_clone_url" "$work_root"
  assert_clone_arg_literal "default_branch" "$default_branch" "$work_root"
  assert_clone_arg_literal "issue_or_pr" "$issue_or_pr" "$work_root"
  ...
  echo "script_pack_version=${SCRIPT_PACK_VERSION:-${CDKBOT_SCRIPT_PACK_VERSION:-unknown}}"
}
