#!/usr/bin/env bash
# Run N sequential cdk-bot webhook triggers; exit when N PRs created or on hard failure.
set -euo pipefail
...
for entry in "${cases[@]}"; do
  ...
  trace_id="$(psql "${PG_URL:-postgres://guild:guild@localhost:5433/guild_db?sslmode=disable}" -t -A -c \
    "SELECT trace_id FROM execution_agent_summaries WHERE workflow_name='cdk-app-update' ORDER BY created_at DESC LIMIT 1;" | tr -d '[:space:]')"
