...
resource "sg_workflow" "infrastructure_audit" {
  ...
  stage_bindings = [
    { stage_id = "fetch-state", agent_ref = sg_agent.stackgen_expert.name, runbook_refs = [sg_runbook_sop.stackgen_audit.name] },
    { stage_id = "evaluate", agent_ref = sg_agent.stackgen_expert.name, stage_depends_on = ["fetch-state"] },
  ]
}
