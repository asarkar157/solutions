...
Blocker comment command (copy verbatim — ONE execute_series):
  export WORK_ROOT="{{work_root}}" REPO_FULL_NAME='<repository_full_name from read_notes>' ISSUE_OR_PR='<issue_or_pr_number from read_notes>' BLOCKER_DETAIL='<clone_blocker + stage_summary:intake-clone-bootstrap from read_notes>' SPECSYM_ALLOW_DIRECT=1; ${local.specsym_pack_ensure_shell_body} && ${local.specsym_pack_dir}/blocker-comment.sh
...
Tracker update command (when pr_url or ci_status in notes):
  export WORK_ROOT="{{work_root}}" REPO_FULL_NAME='<repository_full_name>' ISSUE_OR_PR='<issue_or_pr_number>' COMMENT_BODY='pr_url=<pr_url> ci_status=<ci_status>' SPECSYM_ALLOW_DIRECT=1; ${local.specsym_pack_ensure_shell_body} && ${local.specsym_pack_dir}/blocker-comment.sh
EOT

  specsym_spawn_context_clone = <<-EOT
${local.specsym_spawn_context_header}
Clone command (copy verbatim — MUST start with `export` so clone-pack.sh inherits WORK_ROOT/REPO_CLONE_URL from env; NO positional path args):
  export WORK_ROOT='{{work_root}}' REPO_CLONE_URL='<repository_clone_url from read_notes>' DEFAULT_BRANCH='<repository_default_branch from read_notes>' ISSUE_OR_PR='<issue_or_pr_number from read_notes>' SPECSYM_ALLOW_DIRECT=1; ${local.specsym_pack_ensure_shell_body} && ${local.specsym_pack_dir}/clone-pack.sh clone
The host already substituted {{work_root}} above into the literal WORK_ROOT value — copy that value exactly (clone-pack.sh normalizes any $HOME prefix). On specsym_pack_error=: note clone_blocker=missing_script_pack.
FORBIDDEN in commands[0].command: clone-pack.sh clone with positional path args; rewriting or hand-rolling the WORK_ROOT value; CLONE_ONE_LINER; manual git clone; clone_info blob instead of repo_clone_path note.
EOT

  specsym_spawn_context_bootstrap = <<-EOT
${local.specsym_spawn_context_header}
Spec-bootstrap command (copy verbatim — starts with `export`; use DOUBLE QUOTES around the {{work_root}} paths so the shell expands $HOME to a real absolute path for stage-runner.sh and spec-bootstrap.sh):
  export WORK_ROOT="{{work_root}}" SPECSYM_ALLOW_DIRECT=1 SDD_FRAMEWORK=${var.sdd_framework} CHANGE_TYPE=${var.change_type}; ${local.specsym_pack_ensure_shell_body} && ${local.specsym_pack_dir}/stage-runner.sh spec-bootstrap "{{work_root}}" "{{work_root}}/repo" '${var.sdd_framework}' '${var.change_type}'
EOT

  specsym_spawn_context_author_spec = <<-EOT
${local.specsym_spawn_context_header}
Author-spec command (shell scaffold from ticket):
  export WORK_ROOT="{{work_root}}" SPECSYM_ALLOW_DIRECT=1 SDD_FRAMEWORK=${var.sdd_framework} CHANGE_TYPE=${var.change_type} FEATURE_ID='<from read_notes issue_or_pr>' ISSUE_TITLE='<from webhook>' ISSUE_BODY='<from webhook>' ISSUE_OR_PR='<issue_or_pr_number>'; ${local.specsym_pack_ensure_shell_body} && ${local.specsym_pack_dir}/stage-runner.sh author-spec "{{work_root}}" "{{work_root}}/repo" '${var.sdd_framework}' '${var.change_type}'
...
Cursor author-spec command (ONE execute_series):
  export WORK_ROOT="{{work_root}}" SPECSYM_ALLOW_DIRECT=1 FEATURE_ID='<feature id>' ISSUE_TITLE='<title>' ISSUE_BODY='<body>'; ${local.specsym_pack_ensure_shell_body} && ${local.specsym_pack_dir}/stage-runner.sh cursor-author-spec "{{work_root}}" "{{work_root}}/repo" '<feature_id>' '<title>' '<body>'
...
Cursor implement command (ONE execute_series — copy verbatim, set spec_tasks_path from read_notes):
  export WORK_ROOT="{{work_root}}" SPECSYM_ALLOW_DIRECT=1 ISSUE_OR_PR='<issue_or_pr_number>'; ${local.specsym_pack_ensure_shell_body} && ${local.specsym_pack_dir}/stage-runner.sh cursor-implement "{{work_root}}" "{{work_root}}/repo" '<spec_tasks_path from notes or tasks.md>' '<issue_or_pr_number>'
...
${local.specsym_spawn_context_header}
Validate command:
  export WORK_ROOT="{{work_root}}" SPECSYM_ALLOW_DIRECT=1; ${local.specsym_pack_ensure_shell_body} && ${local.specsym_pack_dir}/stage-runner.sh validate "{{work_root}}" "{{work_root}}/repo" '<pr_number or empty>'
...
${local.specsym_spawn_context_header}
Commit-pr command:
  export WORK_ROOT="{{work_root}}" REPO_FULL_NAME='<repository_full_name>' ISSUE_OR_PR='<issue_or_pr_number>' SPECSYM_ALLOW_DIRECT=1; ${local.specsym_pack_ensure_shell_body} && ${local.specsym_pack_dir}/stage-runner.sh commit-pr "{{work_root}}" "{{work_root}}/repo" '<repository_default_branch>' '<issue_or_pr_number>'
...
${local.specsym_spawn_context_header}
Archive command:
  export WORK_ROOT="{{work_root}}" SPECSYM_ALLOW_DIRECT=1; ${local.specsym_pack_ensure_shell_body} && ${local.specsym_pack_dir}/stage-runner.sh archive "{{work_root}}" "{{work_root}}/repo" '${var.sdd_framework}'
EOT
  ...
  spawn_contracts_intake_clone = [
    {
      ...
      timeout_seconds     = 300
      goal                = "Clone ONLY — max 2 tools: read_notes then ONE ${local.shell_tool_prefix}_execute_series. read_notes repository_clone_url repository_default_branch issue_or_pr_number. Command MUST copy the spawn-context Clone line **character-for-character**, starting with `export WORK_ROOT='{{work_root}}'` (the host provides {{work_root}} verbatim — do NOT rewrite or hand-roll the path; clone-pack.sh normalizes it). Fill REPO_CLONE_URL/DEFAULT_BRANCH/ISSUE_OR_PR from notes, then clone-pack.sh clone with NO path arguments. note repo_clone_path repo_head_sha from stdout. On empty execute_series output: retry up to 3× (15s) before clone_blocker=runner_unavailable."
