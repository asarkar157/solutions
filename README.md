...
- **StackGen** platform with Guild enabled, and **terraform-provider-stackgen** **`>= 0.1.13, < 0.2.0`** from `releases.stackgen.com` (matches module pins; includes evidence checklists, remediation `approve`, and remote runner attach) ([provider reference docs](https://appcd-dev.github.io/terraform-provider-stackgen/))
