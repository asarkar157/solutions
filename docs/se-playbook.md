...
| "How does the agent remember past incidents?" | **`episodic-memory`** | Two-act chat demo: `memory_store` in session A, `memory_search` in a new thread session B. No integrations — teaches property glossary and pull-based recall gotchas. For production `shared:incidents`, follow with **`incident-triage`**. |
